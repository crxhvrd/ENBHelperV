...
// Returns plain game version info from the game's executable properties
...
static_assert(sizeof(GameVersionInfo) == 8);

IMPORT GameVersionInfo getGameVersionInfo();

// Returns internal game version index, this is here only for compatibility
// with the existing code, use getGameVersionInfo() instead
